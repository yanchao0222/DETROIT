def func_eval_imp_perf(filled_value_sample, metric = 'RMSE range norm'):
    ...
    lab_data_all_to_be_filled, gt_data_all, lab_data_len, gt_data_len, need_be_filled_all = np.load('./train_groundtruth/lab_data.npy')
    ...
    performance = [np.sqrt(error[idx] / number_missing[idx]) for idx in range(1,len(error))]
    return performance
